...
object PayDay extends Serializable with StaticsTrait {
  ...
  def statics(platformID: String, today: String) = {
    ...
    for ((hostID, payInfo) <- userPayMap) {
      ...
      val currency = payInfo._1
      val userCashNum = payInfo._2
      val totalNum = payInfo._3
      val totalGold = payInfo._4
      var cashNum = userCashNum
      var totalCashNum = userCashNum
      var payUserNum = payInfo._5
      payResults("PayGold") = payResults("PayGold") + totalGold
      payResults("PayNum") = payResults("PayNum") + totalNum
      payResults("PayUserNum") = payResults("PayUserNum") + payUserNum
  ...
  def loadUserPayDayFromDB(platformID: String, date: String) = {
    ...
    payRes.select("HostID", "Currency", "TotalCashNum", "TotalNum", "TotalGold").rdd.map(row => {
      ...
      var currency = "RMB" //默认是美金
      var payUserNum = 0
      var totalCashNum = BigDecimal(0)
      var totalNum = 0
      var totalGold = 0
      for (y <- x) {
        currency = y._1
      ...
      (currency, totalCashNum, totalNum, totalGold, payUserNum)
    }).collectAsMap
  }
  /**
